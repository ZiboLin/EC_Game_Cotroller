package tracks.singlePlayer.testing.Several_Step_Look_Ahead;
...
public class Agent extends AbstractPlayer {
    ...
    public Types.ACTIONS act(StateObservation stateObs, ElapsedCpuTimer elapsedTimer) {
        ...
        for (Types.ACTIONS action : stateObs.getAvailableActions()) { //For all available actions. 循环所有可能的actions
            ...
            //     bestAction = action;
            // }
            //next action within current action 
            // for (Types.ACTIONS action2: stateObs.getAvailableActions()) { //For all available actions. 循环所有可能的actions
            //     StateObservation stCopy2 = stateObs.copy();  //Copy the original state (to apply action from it) stCopy这里是当前action的state
            //     stCopy2.advance(action2);                     //Apply the action. Object 'stCopy' holds the next state. 执行当前action
            //     double Q2 = heuristic.evaluateState(stCopy); //Get the reward for this state.
            //     if (Q2 > maxQ) { 
            //         maxQ = Q2;
            //         bestAction = action2;
            //     }
